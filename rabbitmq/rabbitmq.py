...
class Rabbitmq:
    ...
    def __del__(self):
        ...
        if self.channel is not None:
            if not self.channel.is_closed and not self.connection.is_closed: # type: ignore
                self.close()
    ...
    def send_message(self, routing_key, message, properties=None):
        self.channel.basic_publish(exchange=self.exchange_name, # type: ignore
    ...
    def get_message(self, queue_name):
        (method, properties, body) = self.channel.basic_get(queue=queue_name, auto_ack=True) # type: ignore
    ...
    def declare_local_queue(self, routing_key):
        ...
        result = self.channel.queue_declare(queue="", exclusive=True, auto_delete=True) # type: ignore
    ...
    def queues_delete(self):
        ...
        for name in self.queue_name:
            ...
            self.channel.queue_unbind(queue=name, exchange=self.exchange_name) # type: ignore
            self.channel.queue_delete(queue=name) # type: ignore
    ...
    def subscribe(self, routing_key, on_message_callback):
        ...
        self.channel.basic_consume(queue=created_queue_name, # type: ignore
